...
const char *IFCONFIG_START_TOKEN = "ifconfig:",
           ...
           *MULTIROUTE_START_TOKEN = "multiroute:",
           *MULTIROUTE_END_TOKEN = "multirouteend.";
...
//                PRINTF("Filtered File 2 created:\n%s\n", routeFile);
            ...
            // copy into route filtered chararray
            if (streq(file + charpointer, MULTIROUTE_START_TOKEN)) {
                multirouteFile = createFilteredFile(file,
                            charpointer,
                            MULTIROUTE_END_TOKEN);
//                PRINTF("Filtered File 2 created:\n%s\n", routeFile);
            }
        }
    }
    ...
    if(multirouteFile)
        parseMultiRouting(multirouteFile);
    delete[] ifconfigFile;
    delete[] routeFile;
    delete[] multirouteFile;
    return 0;
}
...
void RoutingTableParser::parseMultiRouting(char *routeFile)
{
    char *str = new char[MAX_ENTRY_STRING_SIZE];
    int pos = strlen(MULTIROUTE_START_TOKEN);
    skipBlanks(routeFile, pos);
    while (routeFile[pos] != '\0') {
        // 1st entry: Host
        pos += strcpyword(str, routeFile + pos);
        ...
        //source netmask groups metric parent(source interface) children(dest interface)
        route->setOrigin(Ipv4Address(str));// source 
        // 2nd entry: netmask
        pos += strcpyword(str, routeFile + pos);
        skipBlanks(routeFile, pos);
        route->setOriginNetmask(Ipv4Address(str)); // netmask
        // 3rd entry: groups
        pos += strcpyword(str, routeFile + pos);
        skipBlanks(routeFile, pos);
        route->setMulticastGroup(Ipv4Address(str)); // groups
        ...
        route->setInInterface(new Ipv4MulticastRoute::InInterface(ie));

        // 6th entry: child
        opp_string interfaceName2;
        interfaceName2.reserve(MAX_ENTRY_STRING_SIZE);
        pos += strcpyword(interfaceName2.buffer(), routeFile + pos);
        skipBlanks(routeFile, pos);
         NetworkInterface *ie2 = ift->findInterfaceByName(interfaceName2.c_str());
        if (!ie)
            throw cRuntimeError("Syntax error in routing file: 7th column: `%s' is not an existing interface", interfaceName2.c_str());

        route->addOutInterface(new Ipv4MulticastRoute::OutInterface(ie2));
